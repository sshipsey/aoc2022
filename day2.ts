...
const p1 = (lines: `${OpponentThrow} ${SelfThrow}`[]) => {
  const rounds: [OpponentThrow, SelfThrow][] = lines.map(
    (line) => line.split(' ') as [OpponentThrow, SelfThrow]
  );
  ...
  return rounds.reduce(
    (score, round) => roundMap[round[0]][round[1]] + scoreMap[round[1]] + score,
    0
  );
};

const p2 = (lines: `${OpponentThrow} ${RoundResult}`[]) => {
  const rounds: [OpponentThrow, RoundResult][] = lines.map(
    (line) => line.split(' ') as [OpponentThrow, RoundResult]
  );
